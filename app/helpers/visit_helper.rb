module VisitHelper
  ...
  def visit_user_page(visit_records,before,after)
    ...
    if after or before
      if after
        visit_records = visit_records.where("users.id > ?", after)
      elsif before
        visit_records = visit_records.where("users.id < ?", before)
      end
    ...
    present visit_records, with: UserShareEntity, auth_token: params[:auth_token]
